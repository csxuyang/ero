...
for i in range(0, 5):
    ...
    result = subprocess.run(["ore", "--rpc", RPC, "--keypair", id_file, "balance"], capture_output=True, text=True)

    #print(result.stdout.split())
    # 提取奖励数额
    current_reward = float(result.stdout.split()[4]) + float(result.stdout.split()[1])
    #current_statke = float(result.stdout.split()[1])
    ...
    # 添加到表格中
    table.add_row([f"ID {i}", f"{current_reward:.8f}", f"{last_reward:.8f}", f"{reward_difference:.8f}", f"{hourly_earnings:.8f}", f"{daily_earnings:.8f}",f"{sol_balance}",f"{gas}"])
